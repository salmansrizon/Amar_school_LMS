...
describe('SMS Compose + Log (issue #36)', () => {
  ...
  beforeAll(async () => {
    ...
    await ownerA.from('absence_sms_rules').delete().gte('created_at', '2000-01-01')
  ...
  it('a manual compose insert lands with kind=manual and a shared batch_id, scoped to the caller\'s School', async () => {
    ...
    const rows = [
      { school_id: schoolIdA, student_id: studentId, phone: '01700000001', body: 'hello', provider: 'log', kind: 'manual', batch_id: batchId, recipient_label: 'Class 6 / A', segments: 1, status: 'sent' },
      { school_id: schoolIdA, student_id: null, phone: '01700000002', body: 'hello', provider: 'log', kind: 'manual', batch_id: batchId, recipient_label: 'Class 6 / A', segments: 1, status: 'sent' },
    ]
  ...
  it('a manual insert cannot claim another School\'s school_id (RLS insert check)', async () => {
    ...
    const { error } = await ownerB
      .from('sms_log')
      .insert({ school_id: schoolIdA, phone: '01700000009', body: 'x', provider: 'log', kind: 'manual', segments: 1 })
