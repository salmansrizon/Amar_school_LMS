...
create table public.merchant_accounts (
  ...
  settlement_currency text not null default 'BDT' check (settlement_currency = 'BDT'),
  evidence jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (provider, legal_name)
);
...
comment on table public.payment_provider_events is
  'Append-only provider notifications; payload must be redacted before storage, with authentication evidence.';
...
begin
  ...
  if intent.status <> 'pending' then raise exception 'payment intent is not payable (status=%)', intent.status; end if;
