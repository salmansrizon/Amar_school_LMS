...
- **The login-to-Employee mapping is a definer function, not a wider view** (migration 0156, `student_message_repliers()`). The report keys on `employees.id`; a reply records `profiles.id`; a teacher who does not hold the Employees grant still has to see a name. The first draft added `profile_id` to `employee_card` — which `0138_current_employee_helper.sql` had already refused in as many words: *"The answer is not to widen the grant or to add profile_id to employee_card (which would let any staff member map every colleague to their login)."* That reasoning holds, and the draft was reverted rather than argued with. The function is narrow in the way the view could not be: it returns a row only for a login that has actually answered a question in the caller's school, so what it discloses is the authorship the screen is about to print, not a directory.
