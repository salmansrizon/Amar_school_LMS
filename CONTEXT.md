...
**Class Catalogue**:
The complete set of Class Offerings defined for a School (the `class_offerings` table, renamed from `classes` — map #568/#582), whether or not any Student is currently enrolled in each. The source of truth for "what Classes exist"; a Class Offering appears here the moment it's created, before any Student is admitted into it. An entry's identity is its Class name + Section **plus** its Academic Year, Shift and Group Department — since [#593](https://github.com/salmansrizon/Amar_school_LMS/issues/593) a School can legitimately hold two entries sharing a name+section that differ on any of those (a Morning and a Day "Nine - A", or last year's "Nine - A" alongside this year's). Displayed everywhere as `{name} ({group}) - {shift} - {section}`, each segment optional and cleanly omitted when absent; the Class Offerings list also shows and filters by Academic Year once a School spans more than one ([#597](https://github.com/salmansrizon/Amar_school_LMS/issues/597)).
_Avoid_: Class list, classes (the renamed table; ambiguous with the pre-rebuild table name in prose)

**Class Offering**:
One row of the Class Catalogue: a class name + section + optional group/shift for one School in one Academic Year (`class_offerings`, renamed from `classes` — map #568/#582). The unit a Student enrolls into, a Class Teacher is assigned to, and a routine slot belongs to — replaced the earlier model where a bare class name/section pair (no id, no year, no shift) stood in for all of these at once.
_Avoid_: Class (too generic on its own — pair it with "Offering" when the specific row, not the general concept, is meant)

**Academic Year**:
The school year a Class Offering belongs to (`class_offerings.academic_year`, `schools.active_academic_year` — map #568/#582's #570). Defaults to the current calendar year at creation, matching this product's Bangladesh-calendar-year convention (no separate academic-year-start-month concept exists). A School's `active_academic_year` is what a newly created Class Offering is stamped with unless told otherwise; a School Owner advances it with the Start Academic Year action ([#594](https://github.com/salmansrizon/Amar_school_LMS/issues/594)), after which last year's Offerings stay put and next year's are created alongside them. List and report screens that show Offerings default their view to the active year; the Class Offerings list keeps every historical year selectable ([#597](https://github.com/salmansrizon/Amar_school_LMS/issues/597)). Switching the active year is a pure view change — it does not touch Enrollment or Promotion (#570).
_Avoid_: Session, term (imply a sub-year subdivision that doesn't exist in this product)

**Shift**:
An optional dimension of a Class Offering (`class_offerings.shift`, one of the School's `configured_shifts` — map #568/#582's #576-#580), one of `Morning`/`Day`/`Evening`/`Night`. Absent (null) for a School with no Shifts configured, or for a Class Offering predating Shift's introduction — both render identically (the Shift segment of the Class Catalogue label is simply omitted). An Employee can be assigned multiple Shifts (`employee_academic_shifts`); a School Owner narrows what they see school-wide via the Global Shift Selection, a read-only view filter, never an authorization mechanism. **Fixed ([issue #593](https://github.com/salmansrizon/Amar_school_LMS/issues/593))**: `class_offerings`' uniqueness guard now covers Shift, Academic Year, and Group Department together (`class_offerings_identity_unique`, `NULLS NOT DISTINCT`) — a School can now legitimately run two Offerings sharing a name+section, differing by any of the three, the same-class two-intakes-a-day scenario Shift's own planning tickets (#576-#580) always assumed as a motivating case. Eight live consumers resolving "which Offering" via `(name, section)` text were moved onto the current Enrollment first — the fix that had to land before the constraint could widen safely, not after: `class_teacher_profile_for`, the `students` write policy, the `student_subject_option`/`student_routine`/`student_exam_routine`/`student_material` views, the `class_offerings` "student reads own class offering" policy, and `loadExamRosterResults` (`lib/exam-print-data.ts`). Found in four passes, not one — three code-review rounds each surfaced a consumer the pass before had missed — so "every consumer" was never a guarantee, only this ticket's best audit at the time; a `grep` for `class_name = ` / `student_in_class` / text-pair joins against `class_offerings`/`students` is how to re-check. **Two classes of already-known consumer, confirmed with the user and explicitly deferred rather than fixed here**: SMS/Notices/Homework targeting resolved by a stored *text pattern*, not an Offering id — since **fixed** ([issue #595](https://github.com/salmansrizon/Amar_school_LMS/issues/595), map #598: see **Publication Target** below); and nine more `class_name`/`section` roster sites across Exams/Fees/Student-login-provisioning found by the same code-review passes, module-sized rather than single-consumer fixes ([issue #596](https://github.com/salmansrizon/Amar_school_LMS/issues/596)).
_Avoid_: Session, batch (this product's "Shift" specifically means a same-day repeated intake, not a school term or an admission cohort)

**Publication Target**:
Who a Notice/Homework/Lesson-plan/daily-lesson/exam-prep row (`publications`, `kind`-discriminated) reaches, and — since map #598 ([issue #595](https://github.com/salmansrizon/Amar_school_LMS/issues/595)) — exactly one of three **scopes** (`publications.target_scope`, NOT NULL as of Wave 7/#608, retiring the old `target_type` `all`/`specific` split):
- **all** — school-wide; every Student, enrolled or not.
- **exact Offering target** (`offering`) — one Class Offering by id (`class_offering_id`). The id is the whole identity; no Class/Section/Shift/Year is copied alongside it (those could drift). A deleted Offering nulls the id (`ON DELETE SET NULL`) — the publication stays as history but resolves to nobody (#599).
- **broadcast target** (`broadcast`) — a *live* predicate re-evaluated against `class_offerings` every time recipients are resolved, never a frozen snapshot: Class name (`target_class_name`, text — no stable Class-identity id exists) + the Academic Year pinned at compose time (`target_academic_year`, never spans years) + Shift / Group Department / Section each independently NULL ("Any") or a specific value.
One match rule backs every consumer — the Student RLS SELECT policy on `publications` + `student_material`, `task_completion_roster`, `homeworkTargetsOffering` / My Classes, and live SMS recipient resolution — expressed once in SQL (`publication_target_matches_offering`, migration 0195) and mirrored 1:1 in TypeScript (`targetMatchesOffering`, `web/lib/publishing.ts`), kept in lockstep by a shared parity test. SMS reuses the same vocabulary but persists nothing — it resolves live at send time. The pre-#595 "everyone in Section A regardless of Class" shape has no representation in the new contract (a broadcast target always names a Class).
_Avoid_: audience, recipient list (the resolved set of Students, not the stored rule), `target_type`/`specific` (the retired discriminator)

**Student Enrollment**:
The authoritative record of a Student's placement in one Class Offering for one span of time (`student_enrollments` — map #568/#582's #572-#574), replacing the legacy model where `students.class_name`/`section`/`roll_number` were the only record of placement. A Student has at most one *current* (open, `closed_at is null`) Enrollment at a time, pointed at by `students.current_enrollment_id`; every closed Enrollment is kept as history, with an `outcome` (`promoted`/`repeated`/`transferred`/`left`) and its own Roll Number. **No current Enrollment is a valid, visible state** (#569) — an unplaced Student is not an error, just unplaced. `students.class_name`/`section`/`roll_number` are kept as a synced denormalized copy for the long tail of screens not yet moved onto the Enrollment (a deliberate transitional bridge, not dead columns) — see `updateStudent`'s own profile-edit path for the one write path that can let the two drift.
_Avoid_: Admission (the one-time act of creating a Student's *first* Enrollment; Enrollment is the ongoing placement record itself), Class assignment (implies a single mutable field, not a historical record)
