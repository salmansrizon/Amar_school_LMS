...
**Class Catalogue**:
The complete set of Class Offerings defined for a School (the `class_offerings` table, renamed from `classes` — map #568/#582) — one entry per class+section combination that exists, whether or not any Student is currently enrolled in it. The source of truth for "what Classes exist"; a Class Offering appears here the moment it's created, before any Student is admitted into it. Displayed everywhere as `{name} ({group}) - {shift} - {section}`, each segment optional and cleanly omitted when absent.
